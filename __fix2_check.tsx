  ...
  return (
    <div className="min-h-screen" style={{ background: '#17181c' }}>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* Devolutiva por tipo */}
        {assessment.testType === 'DISC' ? (isPrint ? (<DiscPrintReport result={resultData as Record<string, unknown>} />) : (<DiscDevolutiva d={resultData} />)) : null}
        {assessment.testType === 'MBTI' ? (isPrint ? (<MbtiPrintReport result={resultData as Record<string, unknown>} />) : (<MbtiDevolutiva d={resultData} />)) : null}
        {assessment.testType === 'ENNEAGRAM' ? (isPrint ? (<EnneagramPrintReport result={resultData as Record<string, unknown>} />) : (<EnneagramDevolutiva d={resultData} />)) : null}
        {assessment.testType === 'TEMPERAMENT' ? (isPrint ? (<TemperamentPrintReport result={resultData as Record<string, unknown>} />) : (<TemperamentDevolutiva d={resultData} />)) : null}
        {assessment.testType === 'ARCHETYPE' ? (isPrint ? (<ArchetypePrintReport result={resultData as Record<string, unknown>} />) : (<ArchetypeDevolutiva d={resultData} />)) : null}
        {assessment.testType === 'ARCHETYPE_FEMININE' ? (isPrint ? (<ArchetypeFemininePrintReport result={resultData as Record<string, unknown>} />) : (<ArchetypeFeminineDevolutiva d={resultData} />)) : null}
        {assessment.testType === 'LOVE_LANGUAGES' ? (isPrint ? (<LoveLanguagesPrintReport result={resultData as Record<string, unknown>} />) : (<LoveLanguagesDevolutiva d={resultData} />)) : null}
        {assessment.testType === 'CAREER_ANCHOR' ? (isPrint ? (<CareerAnchorPrintReport result={resultData as Record<string, unknown>} />) : (<CareerAnchorDevolutiva d={resultData} />)) : null}
        {assessment.testType === 'EMOTIONAL_INTELLIGENCE' ? (isPrint ? (<EmotionalIntelligencePrintReport result={resultData as Record<string, unknown>} />) : (<EmotionalIntelligenceDevolutiva d={resultData} />)) : null}
