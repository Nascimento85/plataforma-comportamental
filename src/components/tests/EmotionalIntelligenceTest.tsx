...
export default function EmotionalIntelligenceTest({
  ...
  const [answers, setAnswers] = useState<Record<number, EIAnswer>>({})
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900">Seu QE foi mapeado!</h2>
          <p className="text-gray-500 text-sm mt-1">Veja seu radar de competências emocionais:</p>
        </div>
        {resultData && <TestResultCard testType="EMOTIONAL_INTELLIGENCE" result={resultData} />}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-brand-50 border border-brand-200 rounded-xl p-4 text-sm text-brand-800">
        <strong>Bloco {page + 1} de {totalPages}:</strong> Pense em como você reage no dia a dia profissional. Indique o quanto cada afirmação descreve você, de <strong>1 (discordo totalmente)</strong> a <strong>5 (concordo totalmente)</strong>.
      </div>
      ...
      <div className="flex gap-3">
        ...
        {page < totalPages - 1 ? (
          ...
          <button
            ...
            className="btn-primary flex-1"
          >
            {submitting ? 'Enviando…' : '✓ Avaliar meu QE'}
          </button>
        )}
      </div>
    </div>
