...
import { getBigFiveSessionQuestions } from '@/lib/engines'
...
const PAGE_SIZE = 11  // 44 questões / 11 = 4 páginas
...
export default function BigFiveTest({
  ...
  // Sorteio determinístico pelo token: candidato reabre, vê as mesmas perguntas.
  const QUESTIONS = useMemo(() => getBigFiveSessionQuestions(token), [token])

  const [answers, setAnswers] = useState<Record<number, BigFiveAnswer>>({})
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-brand-50 border border-brand-200 rounded-xl p-4 text-sm text-brand-800">
        <strong>Bloco {page + 1} de {totalPages}:</strong> Pense em como você se comporta no ambiente profissional. Indique o quanto cada afirmação descreve você, de <strong>1 (discordo totalmente)</strong> a <strong>5 (concordo totalmente)</strong>.
      </div>
      ...
      <div className="flex gap-3">
        ...
        {page < totalPages - 1 ? (
          ...
          <button
            ...
            className="btn-primary flex-1"
          >
            {submitting ? 'Enviando…' : '✓ Revelar meu estilo de liderança'}
          </button>
        )}
      </div>
    </div>
