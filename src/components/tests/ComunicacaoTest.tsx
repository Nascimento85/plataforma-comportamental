...
import { getCommSessionQuestions } from '@/lib/engines'
import { submitTestWithFallback } from '@/lib/submit-test'

interface CommAnswer {
  questionId: number
...
export default function ComunicacaoTest({
  ...
  const QUESTIONS = useMemo(() => getCommSessionQuestions(token), [token])

  const [answers, setAnswers] = useState<Record<number, CommAnswer>>({})
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-brand-50 border border-brand-200 rounded-xl p-4 text-sm text-brand-800">
        <strong>Bloco {page + 1} de {totalPages}:</strong> Para cada situação, escolha a opção que mais combina com o seu jeito <strong>natural e espontâneo</strong> de se comunicar. Não existe certo ou errado, só a sua preferência. Vá pela primeira reação.
      </div>
          ...
          return (
            <div key={q.id} className="card p-5">
              ...
              <div className="space-y-2">
                {q.opcoes.map((opt, oi) => {
                  const isSel = selected === opt.valor
                  return (
                    <button
                      key={opt.valor}
                      onClick={() => handleAnswer(q.id, opt.valor)}
      ...
      <div className="flex gap-3">
        ...
        {page < totalPages - 1 ? (
          ...
          <button
            ...
            className="btn-primary flex-1"
          >
            {submitting ? 'Enviando…' : '✓ Ver meu Mapa da Comunicação'}
          </button>
        )}
      </div>
    </div>
