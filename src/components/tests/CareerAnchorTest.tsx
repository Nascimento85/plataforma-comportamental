...
export default function CareerAnchorTest({
  ...
  const [answers, setAnswers] = useState<Record<number, CareerAnchorAnswer>>({})
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900">Sua Âncora foi revelada!</h2>
          <p className="text-gray-500 text-sm mt-1">Aqui está o resumo do que move sua carreira:</p>
        </div>
        {resultData && <TestResultCard testType="CAREER_ANCHOR" result={resultData} />}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-brand-50 border border-brand-200 rounded-xl p-4 text-sm text-brand-800">
        <strong>Bloco {page + 1} de {totalPages}:</strong> Leia cada afirmação e indique o quanto ela descreve você no contexto profissional, de <strong>1 (discordo totalmente)</strong> a <strong>5 (concordo totalmente)</strong>.
      </div>
      ...
      <div className="flex gap-3">
        ...
        {page < totalPages - 1 ? (
          ...
          <button
            ...
            className="btn-primary flex-1"
          >
            {submitting ? 'Enviando…' : '✓ Descobrir minha Âncora'}
          </button>
        )}
      </div>
    </div>
