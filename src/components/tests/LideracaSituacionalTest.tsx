...
import { getLsSessionQuestions } from '@/lib/engines'
...
const STYLE_VALUE: Record<'S1' | 'S2' | 'S3' | 'S4', number> = { S1: 1, S2: 2, S3: 3, S4: 4 }
const LETTER = ['A', 'B', 'C', 'D']

const PAGE_SIZE = 4 // 12 cenários / 4 = 3 páginas

export default function LideracaSituacionalTest({
  assessmentId,
  token,
}: {
  assessmentId: string
  ...
  const QUESTIONS = useMemo(() => getLsSessionQuestions(token), [token])

  const [answers, setAnswers] = useState<Record<number, LsAnswer>>({})
  ...
  async function handleSubmit() {
    ...
    if (unanswered.length > 0) {
      setError(`${unanswered.length} situação(ões) ainda não respondida(s).`)
      return
    }
    setSubmitting(true)
    setError('')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-brand-50 border border-brand-200 rounded-xl p-4 text-sm text-brand-800">
        <strong>Bloco {page + 1} de {totalPages}:</strong> Em cada situação, escolha como você <strong>realmente agiria</strong>, não o que parece mais bonito. Pense no seu dia a dia liderando pessoas. Vá pela primeira reação.
      </div>
          ...
          return (
            <div key={q.id} className="card p-5">
              <p className="text-sm font-semibold text-gray-800 mb-4 leading-relaxed">
                <span className="text-xs text-gray-400 mr-2 font-mono">{page * PAGE_SIZE + idx + 1}.</span>
                {q.contexto}
              </p>
              <p className="text-[13px] text-gray-500 mb-3">Como você agiria?</p>

              <div className="space-y-2">
                {q.opcoes.map((opt, oi) => {
                  const val = STYLE_VALUE[opt.estilo]
      ...
      <div className="flex gap-3">
        <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0} className="btn-secondary flex-1">
          ← Anterior
        </button>
        {page < totalPages - 1 ? (
          <button
            onClick={() => {
              if (!pageComplete) { setError('Responda todas as situações deste bloco antes de continuar.'); return }
          ...
          <button onClick={handleSubmit} disabled={submitting || answered < QUESTIONS.length} className="btn-primary flex-1">
            {submitting ? 'Enviando…' : '✓ Ver meu perfil de liderança'}
          </button>
        )}
      </div>
    </div>
