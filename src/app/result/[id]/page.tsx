...
function BulletList({ items, color = '#22c55e' }: { items: string[]; color?: string }) {
  return (
...
function TagRow({ items, bg, textColor }: { items: string[]; bg: string; textColor: string }) {
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item, i) => (
        <span key={i} className="text-sm font-medium px-3 py-1 rounded-full border"
          style={{ backgroundColor: bg, color: textColor, borderColor: textColor + '40' }}>
          {item}
        </span>
      ))}
    </div>
...
function InfoBox({ label, text, bg, labelColor, textColor }: {
  ...
  return (
    <div className="rounded-xl border p-4" style={{ backgroundColor: bg, borderColor: labelColor + '50' }}>
      <p className="text-xs font-bold uppercase tracking-wide mb-1" style={{ color: labelColor }}>{label}</p>
      <p className="text-sm font-medium leading-relaxed" style={{ color: textColor }}>{text}</p>
    </div>
  )
}
  ...
  return (
    <div className="space-y-5">
      ...
      <Card><div className="p-6">
        ...
        {sorted.map(([name, pct]) => (
          <Bar key={name} label={name} pct={pct} color={BRAND} bold={name === r.dominant} />
        ))}
      </div></Card>
...
export default async function PublicResultPage({ params, searchParams }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
          <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-brand-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">PC</div>
      ...
      {isPrint && (
        <div style={{ backgroundColor: '#2a47f5', color: '#fff', padding: '48px 48px 40px' }}>
          <p style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', opacity: 0.7, marginBottom: '12px' }}>
            {APP_NAME} · Avaliação Comportamental
          </p>
          <h1 style={{ fontSize: '28px', fontWeight: 800, margin: '0 0 8px' }}>{testLabel}</h1>
          <p style={{ fontSize: '18px', opacity: 0.85, margin: '0 0 4px' }}>Devolutiva: {assessment.employee.name}</p>
          <p style={{ fontSize: '14px', opacity: 0.6 }}>{assessment.company.name}{finishedAt ? ` · ${finishedAt}` : ''}</p>
        </div>
      )}
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        {!isPrint && (
          <Card>
            <div className="p-5 flex items-start justify-between flex-wrap gap-3">
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wide mb-0.5">Avaliação de</p>
              ...
              <div className="text-right">
                <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-brand-50 text-brand-700 border border-brand-200">
                  {testLabel}
                </span>
                {finishedAt && <p className="text-xs text-gray-400 mt-1">Concluído em {finishedAt}</p>}
              </div>
            </div>
          </Card>
        )}
        ...
        {/* Footer */}
        {!isPrint && (
          <div className="text-center text-xs text-gray-400 pb-6 space-y-2">
            <p>Este relatório é confidencial e destinado exclusivamente ao avaliado e à empresa solicitante.</p>
            <p>Gerado pela <strong className="text-brand-600">{APP_NAME}</strong></p>
          </div>
        )}
      </main>
    </div>
