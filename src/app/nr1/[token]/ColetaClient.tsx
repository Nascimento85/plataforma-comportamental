    ...
    <div className="min-h-screen py-8 px-4"
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-6">
          <span className="inline-block text-[12px] font-bold uppercase tracking-[0.18em] px-3 py-1 rounded-full mb-3"
        ...
        {(etapa !== 'INTRO' && etapa !== 'OK') && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[13px] font-bold uppercase tracking-widest text-soul-ink/80">
                {etapa === 'KARASEK' ? '1 de 3 · Karasek' : etapa === 'ERI' ? '2 de 3 · ERI' : '3 de 3 · COPSOQ'}
              </span>
              <span className="text-[13px] font-bold text-soul-ink/80">{progresso}%</span>
            </div>
        ...
        {etapa === 'INTRO' && (
          <div className="soul-panel space-y-4">
            ...
            <ul className="space-y-2 text-[14px] text-soul-ink/90 font-medium">
        ...
        {etapa === 'FINAL' && (
          <div className="soul-panel space-y-4 text-center">
            ...
            <p className="text-[15px] text-soul-ink/90 font-medium">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={() => setEtapa('COPSOQ')} disabled={enviando}
                      className="flex-1 py-3 rounded-full text-[15px] font-bold border-2"
                      style={{ borderColor: 'rgba(240,236,227,0.42)', color: 'rgba(240,236,227,0.85)' }}>
                Revisar
              </button>
              <button onClick={submeter} disabled={enviando}
                      className="flex-[1.3] py-3 rounded-full text-[15px] font-bold text-white shadow-terra disabled:opacity-60"
                      style={{ background: 'linear-gradient(135deg, #c4633a, #d4943a)' }}>
        ...
        {questoes.map((q, i) => {
          ...
          return (
            <div key={q.id} className="rounded-2xl p-3" style={{ background: 'rgba(196,99,58,0.04)' }}>
              <p className="text-[15px] font-semibold text-soul-ink mb-2">
                ...
                {opcs.map(o => {
                  ...
                  return (
                    <button key={o.v} type="button"
                            ...
                            className="px-3 py-2 rounded-full text-[13.5px] font-bold transition-all"
                            style={{
      ...
      <div className="flex gap-3 pt-2">
        {onAnterior && (
          <button onClick={onAnterior}
                  className="px-4 py-2.5 rounded-full text-[14px] font-bold border-2"
                  style={{ borderColor: 'rgba(240,236,227,0.42)', color: 'rgba(240,236,227,0.85)' }}>
            ← Anterior
          </button>
        )}
        <button onClick={onProximo} disabled={!podeAvancar}
                className="flex-1 py-2.5 rounded-full text-[14px] font-bold text-white disabled:opacity-50"
                style={{ background: 'linear-gradient(135deg, #c4633a, #d4943a)' }}>
