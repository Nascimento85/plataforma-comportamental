    ...
    <div className="space-y-6">
      ...
      <div className="rounded-3xl p-6 md:p-7 relative overflow-hidden"
        ...
        <div className="relative z-10">
          ...
          <p className="text-[13px] font-bold uppercase tracking-widest mt-2" style={{ color: GOLD }}>Team Build</p>
          <h1 className="font-serif text-3xl md:text-4xl font-semibold text-white leading-tight">{teamNome}</h1>
          <p className="text-[15px] text-white/80 font-medium mt-1 max-w-2xl">
      ...
      {analise.comPerfil < 2 ? (
        <div className="soul-panel text-center py-8">
          <p className="text-[15px] text-soul-ink/80 font-medium max-w-lg mx-auto">
                ...
                return (
                  <div key={e}>
                    <div className="flex justify-between items-baseline text-[14px] mb-1">
                      <span className={dominante ? 'font-bold text-soul-ink' : 'font-medium text-soul-ink/88'}>
                        {info.rotulo} <span className="text-soul-ink/70 font-normal">· {info.descricao}</span>
                      </span>
                  ...
                  return (
                    <div key={i} className="rounded-2xl p-4" style={{ background: 'rgba(38,40,46,0.5)', borderLeft: `4px solid ${cor}` }}>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <p className="text-[15.5px] font-bold text-soul-ink">{a.titulo}</p>
                      </div>
                      <p className="text-[14.5px] text-soul-ink/88 font-medium leading-relaxed">{a.diagnostico}</p>
              ...
              {(Object.keys(FASES_TUCKMAN) as FaseTuckman[]).map((f) => {
                ...
                return (
                  <button key={f} onClick={() => salvarFase(f)}
                    ...
                    <p className="text-[14.5px] font-bold" style={{ color: active ? info.cor : '#1c1a17' }}>{info.rotulo}</p>
                    <p className="text-[13px] text-soul-ink/75 font-medium leading-snug mt-0.5">{info.subtitulo}</p>
                  </button>
                )
              })}
            </div>
...
function DinamicaCard({ fase }: { fase: FaseTuckman }) {
  ...
  return (
    <div className="rounded-2xl p-5" style={{ background: `${info.cor}0d`, border: `1px solid ${info.cor}40` }}>
      <p className="text-[13px] font-bold uppercase tracking-widest" style={{ color: info.cor }}>
      ...
      <p className="text-[14.5px] text-soul-ink/88 font-medium mt-2 leading-relaxed">{d.objetivo}</p>
      <p className="text-[13.5px] font-bold uppercase tracking-wide text-soul-ink/72 mt-3">Duração: {d.duracao}</p>

      <p className="text-[13.5px] font-bold uppercase tracking-wide text-soul-ink/80 mt-4 mb-2">Passo a passo</p>
      <ol className="space-y-2">
        {d.passos.map((p, i) => (
          <li key={i} className="flex items-start gap-2.5">
            <span className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-white text-[13px] font-bold mt-0.5" style={{ background: info.cor }}>{i + 1}</span>
            <span className="text-[14.5px] text-soul-ink/90 font-medium leading-relaxed">{p}</span>
          </li>
        ))}
      </ol>

      <div className="rounded-xl px-4 py-3 mt-4 text-[14px] font-medium"
    ...
    <div className="rounded-2xl p-4" style={{ background: 'rgba(38,40,46,0.5)', border: '1px solid rgba(58,61,69,0.8)' }}>
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <div className="min-w-0">
          <p className="text-[15px] font-bold text-soul-ink leading-tight truncate">{member.nome}</p>
      ...
      {manual ? (
        <div className="space-y-2.5">
          <div>
            <p className="text-[12px] font-bold uppercase tracking-wide" style={{ color: perfil?.cor }}>Funciono melhor quando</p>
            <p className="text-[13.5px] text-soul-ink/88 font-medium leading-snug">{manual.funcionoMelhorQuando}</p>
          </div>
          <div>
            <p className="text-[12px] font-bold uppercase tracking-wide text-rose-700">Me desmotivo se</p>
            <p className="text-[13.5px] text-soul-ink/88 font-medium leading-snug">{manual.desmotivadoSe}</p>
          </div>
          <div>
            <p className="text-[12px] font-bold uppercase tracking-wide text-soul-ink/78">Para resolver um conflito comigo</p>
            <p className="text-[13.5px] text-soul-ink/88 font-medium leading-snug">{manual.resolverConflito}</p>
          </div>
        </div>
      ) : (
        <p className="text-[13.5px] text-soul-ink/72 font-medium">Defina o perfil DISC na matriz para gerar o manual.</p>
      )}
    </div>
  )
}
    ...
    <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center px-4 py-6 overflow-y-auto"
      ...
      <div className="bg-soul-parchment rounded-3xl shadow-soul-xl w-full max-w-lg p-6 md:p-7" style={{ border: '1px solid rgba(58,61,69,0.6)' }}>
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} aria-label="Fechar" className="w-9 h-9 rounded-full flex items-center justify-center text-soul-ink/80 hover:bg-soul-mist/60 text-2xl leading-none">×</button>
        </div>

        <div className="space-y-5">
          {DIAGNOSTICO_TUCKMAN.map((q, qi) => (
            <div key={qi}>
              <p className="text-[15px] font-bold text-soul-ink mb-2">{qi + 1}. {q.pergunta}</p>
              <div className="space-y-1.5">
                {q.opcoes.map((opt, oi) => {
                  const sel = respostas[qi] === opt.fase
                  return (
                    <button key={oi} onClick={() => setRespostas((p) => ({ ...p, [qi]: opt.fase }))}
                            className="w-full text-left rounded-xl px-3 py-2.5 text-[14px] font-medium transition-all"
        ...
        <button onClick={calcular} disabled={!completo}
                className="w-full mt-6 py-3 rounded-full text-[15px] font-bold text-white shadow-terra disabled:opacity-60"
